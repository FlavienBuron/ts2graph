...
class SpatialDecoder(nn.Module):
    def __init__(
        ...
        self.graph_conv = SpatialConvOrderK(
            c_in=d_model,
            c_out=d_model,
            support_len=support_len * order,
            order=1,
            include_self=False,
        )
        if attention_block:
            self.spatial_att = SpatialAttention(
                d_in=d_model, d_model=d_model, nheads=nheads, dropout=dropout
            )
    ...
    def forward(self, x, m, h, u, adj, cached_support=False):
        ...
        if self.order > 1:
            if cached_support and (self.adj is not None):
                ...
                adj = SpatialConvOrderK.compute_support_orderK(
                    adj, self.order, include_self=False, device=x_in.device
                )
        ...
        if self.spatial_att is not None:
            ...
            out_att = self.spatial_att(
                x_in, torch.eye(x_in.size(2), dtype=torch.bool, device=x_in.device)
            )
...
class GRIL(nn.Module):
    def __init__(
        ...
        # Fist stage readout
        self.first_stage = nn.Conv1d(
            in_channels=self.hidden_size, out_channels=self.input_size, kernel_size=1
        )
    ...
    def init_hidden_states(self, n_nodes):
        ...
        for l in range(self.n_layers):
            ...
            vals = torch.distributions.Normal(0, std).sample(
    ...
    def get_h0(self, x):
        ...
        return [
            torch.zeros(size=(x.shape[0], self.hidden_size, x.shape[2])).to(x.device)
        ] * self.n_layers
    ...
    def forward(self, x, adj, mask=None, u=None, h=None, cached_support=False):
        ...
        *_, steps = x.size()
        ...
        for step in range(steps):
            ...
            print(f"DEBUG: {type(x_s)=} {type(m_s)=} {type(xs_hat_1)=}")
...
class BiGRIL(nn.Module):
    ...
    def forward(self, x, adj, mask=None, u=None, cached_support=False):
        ...
        # Forward
        fwd_out, fwd_pred, fwd_repr, _ = self.fwd_rnn(
            x, supp, mask=mask, u=u, cached_support=cached_support
        )
        # Backward
        rev_x, rev_mask, rev_u = [reverse_tensor(tens) for tens in (x, mask, u)]
        *bwd_res, _ = self.bwd_rnn(
            rev_x, supp, mask=rev_mask, u=rev_u, cached_support=cached_support
        )
        ...
        if self._impute_from_states:
            ...
            if self.emb is not None:
                ...
                inputs += [
                    self.emb.view(1, *self.emb.shape, 1).expand(b, -1, -1, s)
                ]  # stack emb for batches and steps
